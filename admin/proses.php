...
if(isset($_GET['kategori'])){
  if($_GET['kategori']=="siswa"){
    ...
    // if (str_contains($_POST[nama], ' ')) { 
    //     $str_nama = str_replace(" ","",$_POST[nama]);
    // }else{
    //   $str_nama = $_POST[nama];
    // }
    $str_nama = substr($_POST[nama],1,2);
    $get_password = generateUsername($str_nama);
    $query_daftar=mysqli_query($konek,"INSERT INTO siswa (nama, jenis_kelamin, kelas, alamat, tgl_lahir, telepon,thn_pendaftaran,username,password) VALUES
    ('$_POST[nama]','$_POST[jenis_kelamin]',$_POST[kelas], '$_POST[alamat]', '$_POST[tgl_lahir]', '$_POST[telepon]','$_POST[thn_pendaftaran]','$get_username','$get_password');");
      if($query_daftar){
